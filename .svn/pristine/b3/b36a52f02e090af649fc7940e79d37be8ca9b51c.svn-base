...
$(document).ready(function(){
	  ...
	  $("#bu1").click(function(){
			...
			var pattern = /^\w$/; 
			var nicheng= pattern.test( $("#uname").val()); 
			if(nicheng==null||nicheng==""||nicheng==0){
				$("#err1").text("用户名不能为空！");
				s = false;
			}else if(s=true){
			$("#err1").text("");
				...
				<div id="tab-2">
					<form action="/Project_kekexinyi/add.ds" method="post" id="form1">
						...
						<ul>
							...
							<li>用户名<span id="err1"></span> <br/>
							<input type="text" name="uname" id="uname"><br/></li>
							...
							<li><br/>
								<input type="button" value="提交注册" id="bu1"></li>
						</ul>
					</form>
				</div>
				</section>
